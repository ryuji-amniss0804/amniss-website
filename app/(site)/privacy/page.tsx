...
/** 1段落。文字列の配列は箇条書き。`{ h3 }` は節の頭に置く小見出し */
type Block = string | string[] | { h3: string };
...
const POLICY_SECTIONS: { title: string; content: Block[] }[] = [
  ...
  },
  {
    ...
    content: [
      ...
      // リンクにはしない。このページには <a> が1つも無く、URLは地の文で出すのが既存の形。
      //
      // 61_ga4_fix：「アクセス解析ツールについて」は**段落ではなく <h3>**にした。
      // このページに小見出しが無かったのは、小見出しが要る内容が無かったからで、
      // 外部サービスの節が第4条の中に並ぶのは今回が初めて。合わせる先がそもそも無い。
      // 判断の基準は「読む人がそこを目印にして移動するかどうか」。ここは目印にする。
      // 段落のままだと、読む人には太字でもない1行にしか見えず、節の頭だと伝わらない。
      { h3: "アクセス解析ツールについて" },
